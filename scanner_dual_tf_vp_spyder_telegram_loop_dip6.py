...
Now with softer gates so we don’t miss good windows:
- Primary gates: Trend + Liquidity + Risk/Reward
- Supporting gates (need >= SUPPORT_MIN_PASS): Sweet-spot, Volume, Momentum
- Divergence moves to bonus (confidence only)
- Softer sweet band (SOFT_SWEET_PAD_ATR)
- Easier momentum (any 2 of 5 turns)
- Volume pass if up-vol >= 0.9 * down-vol over lookback
- AVWAP 4H tolerance by ATR(4H)
- Easier price trigger (optional)
...
RAW_TICKERS = ["nvda","amd","orcl","avgo","pltr","net","amzn","googl","msft","klac","ibm","aapl",
               ...
               "mu","crm","tem","rklb","crwd","uvxy","unh","jpm","abt","bynd","race","sofi","dell","upst","sofi",  "gld", "gldm","shny"]
...
# If True => stricter: BUY requires regime + avwap4h + price trigger; else regime becomes advisory
...
# New: extra outer pad (soft band) in ATRs to avoid barely-missed entries
...
def volume_accum_ok(o: pd.DataFrame, lookback=10, mult=1.2) -> Tuple[bool, str]:
    ...
    # relaxed: either heavy up-day OR up-volume dominates recent down-volume
...
def momentum_turn_ok(o: pd.DataFrame) -> Tuple[bool, List[str], int]:
    ...
    ok = (sum(bool(v) for v in votes) >= 2)  # relaxed
    reasons=[]
...
def suggest_levels(o: pd.DataFrame, lookback=180, bins=120):
    ...
    sug_buy  = round(float(np.clip(base, lo, hi + SOFT_SWEET_PAD_ATR*r.ATR14)), 2)  # allow soft pad on sug_buy too
...
def _bullish_candle(o: pd.DataFrame) -> bool:
    ...
    # simple bullish engulfing or hammer-ish
...
def process_one(ticker: str, tf: str, df: pd.DataFrame, vp_lookback=180) -> Row:
    ...
    ok_div, div_msg, div_cnt, div_names = divergence_ok(o, DIV_MIN)  # bonus
    ...
    # New gating: must-have + supporting >= N
    ...
    # Status logic (REGIME_STRICT controls)
    if REGIME_STRICT:
