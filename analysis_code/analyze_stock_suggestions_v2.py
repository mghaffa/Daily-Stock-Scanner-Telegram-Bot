...
- Fetch daily OHLC via yfinance for those tickers and dates.
...
Key upgrades vs v1
- Understands your exact message format from scanner_dual_tf_vp_spyder_telegram_loop_dip10.py:
  - "Entries YYYY-MM-DD HH:MM:"
  - Tier sections (Tier 1 / Tier 2 / Tier 3A / Tier 3B), plus LRC touch reporting blocks
  - Lines like:
      - TICKER [1D] c 123.45 | (2.1% away) | div_v3 4 | conf 70 | W%R -92.3 | StochRSI Delta 7.1
      - TICKER [1D] c 123.45 | div_v3 4 ['RSI','MACD'] | conf 70 | W%R -92.3 | StochRSI Delta 7.1 | sug_buy 120.0 | R/R 6.2
      - TICKER [1D] c 123.45 | LRC_lo 118.0 | stop 115.0 | tgt 135.0 | R/R 5.1 | conf 80
- Adds more features:
  - dist_to_lrc_pct (if lrc_lower or LRC_lo present)
  - ema_gap_pct ((ema50-close)/ema50) if EMA50 present
  - quality flags derived from your intended gates (optional, best-effort)
- Can read your CSV output directly (recommended, it already contains lrc_lower/mid/upper, ema50/ema200,
  rr, conf, status, trigger_ok, primary_ok, strict_ok, div_v3_cnt, willr, stochrsi_delta, etc.)

Install
  pip install pandas numpy yfinance pandas_market_calendars scikit-learn

Run examples
  python analyze_stock_suggestions_v2.py --log stock-info.txt --out_dir out
  python analyze_stock_suggestions_v2.py --csv scanner_dual_tf_vp_dip7.csv --out_dir out
  python analyze_stock_suggestions_v2.py --log stock-info.txt --csv scanner_dual_tf_vp_dip7.csv --out_dir out
...
class ParsedSignal:
    ...
    div_v3_names_raw: Optional[str] = None  # store raw string; you can parse later if desired
    ...
    away_pct_logged: Optional[float] = None  # from "(x% away)" if present
...
def parse_log_text(path: str) -> pd.DataFrame:
    ...
    # Data line: "- TICKER [1D] c 123.45 | ..."
    line_re = re.compile(r"^\s*-\s+([A-Z0-9\.\-]+)\s+\[([0-9A-Za-z]+)\]\s+c\s+([0-9]+(?:\.[0-9]+)?)\s*\|\s*(.*)$")

    # Also handle a shorter style like "- TICKER [1D] c 123.45 | LRC_lo 118.0 | ..."
    # The main regex already covers it.

    # Extractors
    away_re = re.compile(r"\(\s*([-+]?[0-9]+(?:\.[0-9]+)?)%\s+away\s*\)", re.IGNORECASE)

    div_re = re.compile(r"div_v3\s+(\d+)", re.IGNORECASE)
    # div_v3 names can look like: div_v3 4 ['RSI', 'MACD_H', ...]
    ...
    current_status: Optional[str] = None  # BUY/WATCH blocks
    ...
    for raw in text.splitlines():
        ...
        lrc_lower = None
        # Your messages sometimes say "LRC_lo", sometimes "lrc_lower"
    ...
    # If you have both tier and status, prioritize status a bit
    df["pri"] = df["tier_pri"] + df["status_pri"]

    # Dedupe per (sig_date, ticker, tf)
...
def parse_scanner_csv(path: str) -> pd.DataFrame:
    ...
    # Normalize column names you likely have
    col_map = {
        "ticker": "ticker",
        "tf": "tf",
        "date": "sig_date_str",
    ...
    # Add log-style post_date/time placeholders (not used for CSV)
    df["post_date"] = df["sig_date"]
    df["post_time"] = dt.time(0, 0)

    # Build tier: infer if you have your tier outputs stored, otherwise set None
    # If lrc_touch_ok and strict_ok true -> treat as LRC_touch_confirmed
    df["tier"] = None
    ...
    # Status is whatever scanner computed
...
def load_signals(log_path: Optional[str], csv_path: Optional[str]) -> pd.DataFrame:
    ...
    # Feature engineering
    df["stoch_abs"] = df.get("stoch_delta", np.nan).astype(float).abs()
    ...
    # Use away_pct_logged if present
...
def attach_forward_returns(df_signals: pd.DataFrame, ohlc: pd.DataFrame) -> pd.DataFrame:
    ...
    for t, sub in o.groupby("Ticker"):
        ...
        closes = sub[price_col].to_numpy(dtype=float)
    ...
    # Quality check: how close is your log close to the market close on the aligned day
