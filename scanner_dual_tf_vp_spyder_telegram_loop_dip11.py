...
Tier 1 (Optimal: Divergence v3 + LRC Lower):
Requires div_v3 >= 2 (the minimum threshold, DIV_MIN=2 in the code).
But the key is that price is touching or very close to the LRC lower band (within ~2% distance). This is considered "optimal" because it's at a strong support level where reversals are more likely and immediate.
Priority: Highest, as it combines divergence with ideal price action (low risk, high reward potential). Even with the minimum div_v3 (e.g., 2), this setup is prioritized over a high-div setup that's not at support.

Tier 2 (Early / Approaching LRC):
Also requires div_v3 >= 2.
Price is farther from LRC lower (3-8% away), so it's "approaching" but not yet at the optimal entry zone.
Priority: Medium, as it's a good signal but might need more downside before reversing.

Tier 3A (Shallow Trend Pullback, Above LRC):
No minimum div_v3 requirement (the filter doesn't check div_v3_cnt at all).
Focuses on mild pullbacks (close < EMA50 but > EMA200, 2.5-8% below EMA50, and 3-8% above LRC).
Priority: Lower, as it's a trend continuation pullback (not necessarily a reversal). Higher div_v3 might appear here by chance (e.g., if indicators align during the dip), but the setup is less "bottomed out" than Tier 1.

Tier 3B (Deep Trend Pullback / Mean Reversion):
Requires div_v3 >= 1 (lower threshold than Tiers 1/2).
Deeper pullbacks (close < EMA50, >=10% above LRC, often riskier with more room to fall).
Priority: Lowest, as these are speculative mean-reversion plays. They might accumulate more divergences (higher div_v3) because the price has fallen further, giving indicators more time to diverge, but this doesn't make them "better" than Tier 1 setups.
...
Interpreting R/R = 11.73
Value	Interpretation
R/R < 1	Bad trade (risk > reward)
R/R ≈ 1.5–3	Decent
R/R ≈ 3–5	Strong
R/R > 5	Excellent / asymmetric
R/R 11.73	Extremely asymmetri


$env:MARKET_ONLY="false"
$env:TELEGRAM_BOT_TOKEN="8278024097:AAGSt_Xs-sbC9xBLEL7uWX52H5h0YjDF1LA"
$env:TELEGRAM_CHAT_ID=1112263001
...
# Universe
RAW_TICKERS = [
    "AAPL","AAPU","ABT","AGQ","AMD","AMZN","AMZU","ARKQ","ARKX","ARQQ","ASTS","ASTX","AVGO","AVGU",
    "BULZ","BYND","CCJ","CEG","COST","COTG","CRM","CRWD","DDOG","DELL","DOGD","GLD","GLDM","GOOGL","HIMS",
    "HOLO","HOOD","HOOW","IBM","INTC","IONQ","JPM","KLAC","LAES","LLY","LMT","META","MSCI","MSFT","MU","MUU",
    "NET","NEE","NFLX","NFXL","NOW","NOWL","NTRA","NVDA","OKLL","OKLO","ORCL","ORCX","PLTR","PLTU","QBTS",
    "QTUM","QUBT","RACE","RGTI","RKLB","RKLX","ROK","SCHD","SCHX","SHOP","SHNY","SLR","SMCI","SOFI","SOFI",
    "TEM","TER","TQQQ","TSLA","TSLL","TSM","UNH","UPST","UVXY","VOO","VST","XEL","ZBRA"
]
...
LRC_TOUCH_TOL_ATR = _env_float("LRC_TOUCH_TOL_ATR", 0.10)
LRC_TOUCH_REPORT = _env_bool("LRC_TOUCH_REPORT", True)
LRC_TOUCH_REPORT_DEDUP = _env_bool("LRC_TOUCH_REPORT_DEDUP", False)
LRC_TOUCH_REPORT_MAX = _env_int("LRC_TOUCH_REPORT_MAX", 25)
...
print(
    "[cfg] LRC_ENABLE=", LRC_ENABLE, "LRC_LEN=", LRC_LEN, "LRC_DEVLEN=", LRC_DEVLEN,
...
def stoch_rsi(rsi_series, stoch_period=14, k_period=3, d_period=3):
    ...
    stoch = 100 * (rsi_series - min_rsi) / (max_rsi - min_rsi)
...
def compute(df: pd.DataFrame) -> pd.DataFrame:
    ...
    o["STOCHRSI_K"], o["STOCHRSI_D"] = stoch_rsi(o["RSI14"])
...
def linear_regression_channel(series: pd.Series, length: int, dev_mult: float) -> Tuple[float, float, float, float, float]:
    ...
    resid = y - y_hat
    stdev = float(np.sqrt(np.mean(resid ** 2)))
...
def near_lrc_lower(o: pd.DataFrame, pct: float = 0.05) -> bool:
    ...
    """
    mid, upper, lower, slope, _ = linear_regression_channel(o["Close"], LRC_LEN, LRC_DEVLEN)
    if not np.isfinite(lower):
        return False
...
def volume_profile(close: pd.Series, volume: pd.Series, lookback=180, bins=120):
    close = _squeeze_col(close).tail(lookback)
    volume = _squeeze_col(volume).reindex(close.index)
    pmin = float(close.min())
    pmax = float(close.max())
    ...
    thresh_hi = 0.7 * vol_by_bin.max()
    thresh_lo = 0.3 * vol_by_bin.max()
...
def volume_accum_ok(o: pd.DataFrame, lookback=10, mult=1.2) -> Tuple[bool, str]:
    w = o.tail(lookback + 1)
    if len(w) < lookback + 1:
        return False, "vol check: not enough bars"
...
class Row:
    ...
    stochrsi_delta: float | None = None
...
def process_one(ticker: str, tf: str, df: pd.DataFrame, vp_lookback=180) -> Row:
    ...
    if o.empty or len(o) < 210:
        raise ValueError("indicator frame short")
    ...
    # Status logic:
    # WATCH stays based on primary_ok
    # BUY requires LRC-touch + strict gates
    if REGIME_STRICT:
    ...
    reasons += regime_msgs
    if IDE_ENABLE_4H and USE_AVWAP_4W_4H:
        reasons.append("4H AVWAP ok" if avwap4h_ok else "4H AVWAP fail")
    ...
    stochrsi_k = round(float(r.STOCHRSI_K), 1) if "STOCHRSI_K" in o.columns else None
    stochrsi_d = round(float(r.STOCHRSI_D), 1) if "STOCHRSI_D" in o.columns else None
    stochrsi_delta = round((stochrsi_d - stochrsi_k), 1) if stochrsi_k is not None and stochrsi_d is not None else None
...
def key_buy(row) -> str:
    return f"BUY|{row['ticker']}|{row['tf']}|{row['date']}"

def key_watch(row) -> str:
    return f"WATCH|{row['ticker']}|{row['tf']}|{row['date']}"
...
def run_once(first_run: bool = False):
    ...
    LRC_NEAR_PCT_STRICT = 0.02
    LRC_NEAR_PCT_EARLY = 0.08
    ...
    if not div_df.empty and {"lrc_lower", "close"}.issubset(div_df.columns):
        ...
        div_tier3a = df_full.iloc[0:0]
        div_tier3b = df_full.iloc[0:0]

    print("\n🔥 Divergence v3 + LRC LOWER (Tier 1 – Optimal) 🔥")
    if div_tier1.empty:
        print("(none)")
    else:
        print(div_tier1[["ticker", "close", "div_v3_cnt", "div_v3_names", "lrc_lower"]].to_string(index=False))

    print("\n🟡 Divergence v3 (Tier 2 – Early / Approaching LRC) 🟡")
    if div_tier2.empty:
        print("(none)")
    else:
        print(div_tier2[["ticker", "close", "div_v3_cnt", "div_v3_names", "lrc_lower"]].to_string(index=False))

    print("\n🟦 Tier 3A – Shallow Trend Pullback (Above LRC) 🟦")
    if div_tier3a.empty:
        print("(none)")
    else:
        print(div_tier3a[["ticker", "close", "ema50", "ema200"]].to_string(index=False))

    print("\n🟪 Tier 3B – Deep Trend Pullback (Below EMA50) 🟪")
    if div_tier3b.empty:
        print("(none)")
    else:
        print(div_tier3b[["ticker", "close", "ema50", "ema200"]].to_string(index=False))
    ...
    if not div_tier1.empty:
        ...
        for _, r in div_tier1.iterrows():
            div_lines.append(
                ...
                f"conf {r['conf']} | "
                f"W%R {r['willr']} | StochRSI Delta {r['stochrsi_delta']}"
            )
    ...
    # ---------------- NEW: Special analysis-driven Telegram sections ----------------
    def _safe_dist_lrc_pct(row: pd.Series) -> float:
        try:
            close = float(row.get("close", np.nan))
            lrc_lo = float(row.get("lrc_lower", np.nan))
            if not np.isfinite(close) or not np.isfinite(lrc_lo) or lrc_lo == 0:
                return np.nan
            return abs(close - lrc_lo) / abs(lrc_lo)
        except Exception:
            return np.nan

    def _group_labels(ticker: str, tier1_set: Set[str], buyzone_set: Set[str], lrc_set: Set[str]) -> str:
        labels = []
        if ticker in tier1_set:
            labels.append("Tier1")
        if ticker in buyzone_set:
            labels.append("BuyZone")
        if ticker in lrc_set:
            labels.append("LRC_touch")
        return ",".join(labels)

    special_lines: List[str] = []

    tier1_tickers = set(div_tier1["ticker"].astype(str).tolist()) if not div_tier1.empty else set()
    buyzone_tickers = set()
    if not buys.empty:
        buyzone_tickers |= set(buys["ticker"].astype(str).tolist())
    if not watch.empty:
        buyzone_tickers |= set(watch["ticker"].astype(str).tolist())

    lrc_tickers = set()
    if not lrc_only_df.empty:
        lrc_tickers |= set(lrc_only_df["ticker"].astype(str).tolist())
    if not lrc_both_df.empty:
        lrc_tickers |= set(lrc_both_df["ticker"].astype(str).tolist())

    candidate_pool = tier1_tickers | buyzone_tickers | lrc_tickers

    special_df = df_full[df_full.get("error", "").fillna("").eq("")].copy()
    if not special_df.empty:
        special_df["ticker"] = special_df["ticker"].astype(str)
        special_df = special_df[special_df["ticker"].isin(candidate_pool)].copy()
        if not special_df.empty:
            special_df["dist_lrc_pct"] = special_df.apply(_safe_dist_lrc_pct, axis=1)

    # Long buy
    # Candidate pool: Tier1 OR BuyZone OR LRC_touch
    # Numeric filters: conf >= 30, W%R <= -92, RR >= 3, dist_to_lrc <= 6%
    long_buy_df = pd.DataFrame()
    if not special_df.empty:
        long_buy_df = special_df[
            special_df["conf"].fillna(0) >= 30
        ]
        long_buy_df = long_buy_df[
            long_buy_df["willr"].notna() & (long_buy_df["willr"] <= -92)
        ]
        long_buy_df = long_buy_df[
            long_buy_df["rr"].notna() & (long_buy_df["rr"] >= 3)
        ]
        long_buy_df = long_buy_df[
            long_buy_df["dist_lrc_pct"].notna() & (long_buy_df["dist_lrc_pct"] <= 0.06)
        ].copy()
        if not long_buy_df.empty:
            long_buy_df = long_buy_df.sort_values(["conf", "rr", "ticker"], ascending=[False, False, True])

    # 10 day best balanced
    # Candidate pool: Tier1 OR BuyZone OR LRC_touch
    # Numeric filters: conf >= 50, W%R <= -90, dist_to_lrc <= 6%
    ten_day_balanced_df = pd.DataFrame()
    if not special_df.empty:
        ten_day_balanced_df = special_df[
            special_df["conf"].fillna(0) >= 50
        ]
        ten_day_balanced_df = ten_day_balanced_df[
            ten_day_balanced_df["willr"].notna() & (ten_day_balanced_df["willr"] <= -90)
        ]
        ten_day_balanced_df = ten_day_balanced_df[
            ten_day_balanced_df["dist_lrc_pct"].notna() & (ten_day_balanced_df["dist_lrc_pct"] <= 0.06)
        ].copy()
        if not ten_day_balanced_df.empty:
            ten_day_balanced_df = ten_day_balanced_df.sort_values(["conf", "dist_lrc_pct", "ticker"], ascending=[False, True, True])
    ...
    if not long_buy_df.empty:
        ...
        for _, r in long_buy_df.iterrows():
            groups = _group_labels(str(r["ticker"]), tier1_tickers, buyzone_tickers, lrc_tickers)
            special_lines.append(
                f"- {r['ticker']} [{r['tf']}] c {r['close']} | "
                f"groups {groups} | conf {r['conf']} | W%R {r['willr']} | "
                f"R/R {r['rr']} | dist LRC {r['dist_lrc_pct']:.1%}"
            )
    ...
    if not ten_day_balanced_df.empty:
        ...
        for _, r in ten_day_balanced_df.iterrows():
            groups = _group_labels(str(r["ticker"]), tier1_tickers, buyzone_tickers, lrc_tickers)
            special_lines.append(
                f"- {r['ticker']} [{r['tf']}] c {r['close']} | "
                f"groups {groups} | conf {r['conf']} | W%R {r['willr']} | "
                f"dist LRC {r['dist_lrc_pct']:.1%}"
            )
    ...
    lines = [
        f"Entries {datetime.now().strftime('%Y-%m-%d %H:%M')}:",
        "",
        "💡 Good Strategy: R/R = Reward ÷ Risk",
        "• R/R 3–5 → Strong",
        "• R/R > 5 → Excellent / asymmetric",
        "• StochRSI Delta < 15  [smaller better]",
        "• Williams %R < -92",
        "",
    ]
    ...
    if new_buys or new_watch or lrc_lines or div_lines or special_lines:
        notify_all("\n".join(lines))
    save_seen(seen)
